...
export const useImageProcessing = () => {
  ...
  // 处理新图像加载
  const processNewImage = useCallback((imageData: ImageDataInterface) => {
    console.log("处理新图像");
    setLoading(true);
  ...
  // 处理压缩预览结果
  const handleCompressPreviewReady = useCallback((payload: ProcessedPayload) => {
  ...
  // 处理常规图像处理结果
  const handleImageProcessed = useCallback((payload: ProcessedPayload) => {
