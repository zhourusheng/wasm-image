...
const ContrastControls: React.FC<ControlProps> = ({ params, onChange }) => (
  <div className="space-y-2">
    <label className="text-sm font-medium">对比度: {((params.factor as number) || 1).toFixed(1)}</label>
    <Slider
      min={0.1}
      max={3}
      step={0.1}
...
const SaturationControls: React.FC<ControlProps> = ({ params, onChange }) => (
  <div className="space-y-2">
    <label className="text-sm font-medium">饱和度: {((params.factor as number) || 1).toFixed(1)}</label>
    <Slider
      min={0}
      max={3}
      step={0.1}
...
const ColorBalanceControls: React.FC<ControlProps> = ({ params, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <div className="text-sm font-medium text-red-500 mb-2">红色: {(params.red as number) || 0}</div>
        <Slider 
          min={-100} 
          max={100} 
          step={1} 
      ...
      <div>
        <div className="text-sm font-medium text-green-500 mb-2">绿色: {(params.green as number) || 0}</div>
        <Slider 
          min={-100} 
          max={100} 
          step={1} 
      ...
      <div>
        <div className="text-sm font-medium text-blue-500 mb-2">蓝色: {(params.blue as number) || 0}</div>
        <Slider 
          min={-100} 
          max={100} 
          step={1} 
...
const CompressControls: React.FC<ControlProps> = ({ params, onChange }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-sm font-medium mb-2 block">压缩质量: {Math.round(quality * 100)}%</label>
        <Slider 
          min={0.1} 
          max={1} 
          step={0.1} 
      ...
      <div>
        <label className="text-sm font-medium mb-2 block">调整大小: {Math.round(scale * 100)}%</label>
        <Slider 
          min={0.1} 
          max={1} 
          step={0.1} 
      ...
      {params.previewSize && (
        <div className="mt-4 p-3 bg-gray-100 dark:bg-gray-700 rounded">
          ...
          <div className="mt-2">
            <div className="text-sm mb-2">原始: {(params.originalSize as string) || '未知'}</div>
            <div className="text-sm">压缩后: {(params.previewSize as string) || '未知'}</div>
          </div>
        </div>
      )}
    </div>
...
const ParamsPanel: React.FC = () => {
  const { activeTool, toolParams, updateToolParams, setActiveTool } = useEditorStore();
