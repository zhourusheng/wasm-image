...
export const drawImageToCanvas = (
  ...
  return ctx.getImageData(0, 0, image.width, image.height) as ImageDataInterface;
};

/**
 * 从图片元素中提取 ImageData，不修改任何现有的 canvas
 */
export const getImageDataFromImage = (image: HTMLImageElement): ImageDataInterface => {
 ...
 */
export const imageDataToCanvas = (imageData: ImageDataInterface): HTMLCanvasElement => {
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');
 ...
 * 创建 ImageData 的深拷贝
 */
export const cloneImageData = (imageData: ImageDataInterface): ImageDataInterface => {
 ...
 * 将 ImageData 复制到剪贴板
 */
export const copyImageDataToClipboard = async (imageData: ImageDataInterface): Promise<void> => {
  const canvas = imageDataToCanvas(imageData);
...
export const flipImageData = (
  ...
  return ctx.getImageData(0, 0, imageData.width, imageData.height) as ImageDataInterface;
};
...
export const cropImageData = (
  ...
  return ctx.getImageData(0, 0, clampedWidth, clampedHeight) as ImageDataInterface;
};

/**
 * 检查图片是否有效
 */
export const isValidImageData = (imageData: unknown): imageData is ImageDataInterface => {
  return (
    imageData !== null &&
